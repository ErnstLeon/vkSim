...
auto Engine::createCommandBuffers() -> void
{
  ...
  m_commandBuffers =
      commandPool.allocateCommandBuffers({.level = vk::CommandBufferLevel::ePrimary, .count = 1});
}

auto Engine::recordCommandBuffer() -> void
{
  auto *fluidObject = m_scene.getFluid();
  ...
  auto &commandBuffer = m_commandBuffers.front();
  vk::CommandBufferBeginInfo beginInfo{};
  commandBuffer.begin(beginInfo);

  m_voxelizer->recordCommandBuffer(commandBuffer);
